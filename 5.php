...
while(true)
{
    ...
    if (!isset($solar_month_sum[$month])) $solar_month_sum[$month] = 0;
    $solar_month_sum[$month] += $solar_gain;
    if (!isset($solar_month_n[$month])) $solar_month_n[$month] = 0;
    $solar_month_n[$month] ++;
        
    // Add up all contributions to heat input
    $heat_input = $heat_output + $solar_gain; // + 580;
...
for ($month=1; $month<=12; $month++) {
    $average = $solar_month_sum[$month] / $solar_month_n[$month];
    // print $month." ".round($average)."W\n";
}
