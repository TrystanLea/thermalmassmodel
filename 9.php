...
// Battery store
$battery_store = true;
$battery_store_capacity = 5.0;
$battery_store_soc = $battery_store_capacity * 0.5;
...
while(true)
{
    ...
    // Increase available power if there is stored energy in the battery
    if ($battery_store && $battery_store_soc>0.1) $max_heatpump_use = ($balance + 300);
    ...
    {
        if ($balance>0) {
            if ($battery_store_soc<$battery_store_capacity) {
                $battery_store_soc += ($balance * $timestep) / 3600000;
                $balance = 0;
            }
        } else {
            if ($battery_store_soc>0) {
                $battery_store_soc += ($balance * $timestep) / 3600000;
                $balance = 0;
            }
        }
    ...
    if ($balance>0) {
        $excess += (1 * $balance * $timestep) / 3600000;
    } else {
        $unmet += (-1 * $balance * $timestep) / 3600000;
    }
...
print "Excess: $excess kWh\n";
print "Unmet: $unmet kWh\n";
